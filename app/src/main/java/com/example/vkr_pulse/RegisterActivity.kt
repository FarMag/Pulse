...
// Рабочая версия регистрации, где пользователю не приходит сообщение на почту, а сразу переходит на экран Qwiz
...
// Данный код работает с регистрацией пользователя по почте (когда пользователю приходит уведомление на почту, что нужно подтвердить создание аккаунта)
                ...
                runOnUiThread {
                    responseData?.let {
                        try {
                            ...
                            when {
                                jsonResponse.has("message") && jsonResponse.getString("message") == "Success" -> {
                                    showSuccessToast("Перейдите на почту и подтвердите создание аккаунта!")
//                                    val accessToken = jsonResponse.getString("access_token")
//                                    val refreshToken = jsonResponse.getString("refresh_token")
//                                    saveTokens(accessToken, refreshToken)
