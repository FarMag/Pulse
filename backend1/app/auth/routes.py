    ...
    if all([email, password, username, birth_date_str, gender]):
        if db.db_connect():
            try:
                ...
                existing_user = db.session.query(User).filter_by(email=encrypted_email).first()
                if existing_user:
...
def login():
    ...
    if 'email' in request.form and 'password_hash' in request.form:
        if db.db_connect():
            try:
                encrypted_email = encrypt_email(request.form['email'])
                user = db.session.query(User).filter_by(email=encrypted_email).first()
                if user:
                    dbpassword = user.password_hash.encode('utf-8')
                    if bcrypt.checkpw(request.form['password_hash'].encode('utf-8'), dbpassword):
                        if user.height is not None:
