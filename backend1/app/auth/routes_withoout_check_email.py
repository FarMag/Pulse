...
# Данный файл не отправляет сообщение на почту о регистрации пользователя.
# Если нужно, что бы данный код работал нужно вернуть в файле RegisterActivity.kt первый закомментрованный код (примерно с первой строчки до 300).
# Там будет сообщение: "Рабочая версия регистрации, где пользователю не приходит сообщение на почту, а сразу переходит на экран Qwiz"
# Нужно раскомментровать весь код ниже и закомментровать код в файле auth/routes.py
