...
get_name_training_plan_bp = Blueprint('get_name_training_plan', __name__)
...
@get_name_training_plan_bp.route('/getNameTrainingPlan', methods=['POST'])
def get_name_training_plan():
    db = DataBase()
    if request.form.get('plan_id'):
        plan_id = request.form.get('plan_id')  # Получаем plan_id из запроса
        if db.db_connect():
            try:
                # Находим план тренировки по plan_id
                training_plan = db.session.query(Training_plans).filter(Training_plans.id == plan_id).first()

                if training_plan:
                    training_plan_dict = training_plans_log_to_dict(training_plan)  # Преобразование в словарь
                    response = jsonify(training_plan_dict)
                    response.data = json.dumps(training_plan_dict, ensure_ascii=False)
                    response.headers.add('Content-Type', 'application/json; charset=utf-8')
                    return response, 200
                else:
