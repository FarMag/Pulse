...
add_user_today_training_bp = Blueprint('add_today_user_training', __name__)
get_user_today_training_bp = Blueprint('get_user_today_training', __name__)
...
@get_user_today_training_bp.route('/getUserTodayTraining', methods=['POST'])
def get_user_today_training():
    ...
    if db.db_connect():
        try:
            ...
            # today = datetime.today().date() # если поле date — это date
            today = datetime.now().strftime('%Y-%m-%d')
            workout = db.session.query(Workouts).filter_by(user_id=user_id, date=today).first()
            if workout:
                duration_time = workout.duration_time or 0
                return {"duration_time": duration_time}, 200
            else:
                return {"duration_time": 0}, 200  # Нет тренировки на сегодня
...
def parse_timer_to_minutes(timer_text):
    # Пример: "00:35:12" → 35.20
...
@add_user_today_training_bp.route('/addTodayUserTraining', methods=['POST'])
def add_today_user_training():
    db = DataBase()
    access_token = request.form.get('access_token')
    timer_text = request.form.get('timer')  # Например: "00:35:12"
    plan_id = request.form.get('planId')

    if not all([access_token, timer_text, plan_id]):
    ...
    try:
        ...
        duration_minutes = parse_timer_to_minutes(timer_text)  # <-- вот тут конвертация!

        print(user_id, plan_id, current_date, duration_minutes)

        new_workout = Workouts(
            user_id=str(user_id),
            plan_id=str(plan_id),
            date=current_date,
            duration_time=str(duration_minutes),  # В виде строки, например "35.20"
            notes=None
        )
        ...
        return jsonify({'success': True, 'message': 'Тренировка добавлена'}), 200
