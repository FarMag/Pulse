...
def run_auth():
    auth_app.run(host='192.168.1.4', port=8001, debug=True, use_reloader=False)  # Отключаем авто-перезапуск

def run_test():
    test_app.run(host='192.168.1.4', port=8002, debug=True, use_reloader=False)  # Отключаем авто-перезапуск
